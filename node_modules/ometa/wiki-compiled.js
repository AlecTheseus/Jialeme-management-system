function readFile(name) {
  ...
  new Ajax.Request("projects/" + name + ".txt", {
    ...
    onException: function onException(x) {
      console.log(x);
    }
  });
  return r;
...
function writeFile(name, text) {
  ...
  if (!ok) throw "unable to write file '" + name + "'";
}

function projectIsDirty() {
  return $('workspaceForm').source.value != $('workspaceForm').source.origValue;
}
dirtyAreYouSureMessage = "The changes you have made to this project will be lost unless you press 'cancel' " + "and save your work. Proceed?";

window.onbeforeunload = function () {
  if (projectIsDirty()) return dirtyAreYouSureMessage;
};
...
function loadProject() {
  ...
  if (projectIsDirty() && !confirm(dirtyAreYouSureMessage)) return;
...
hashChangedHandler = function (_hashChangedHandler) {
  function hashChangedHandler() {
    return _hashChangedHandler.apply(this, arguments);
  }

  hashChangedHandler.toString = function () {
    return _hashChangedHandler.toString();
  };

  return hashChangedHandler;
}(function () {
  if (document.location.hash == hashChangedHandler.oldHash) return;
...
//# sourceMappingURL=wiki-compiled.js.map