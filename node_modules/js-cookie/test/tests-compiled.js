...
QUnit.test('Call to read cookie when there is a window.json variable globally', function (assert) {
	...
	assert.strictEqual(_typeof(Cookies.get('boolean')), 'string', 'should not change the returned type');
...
QUnit.test('expires option as fraction of a day', function (assert) {
	...
	var findValueForAttributeName = function findValueForAttributeName(createdCookie, attributeName) {
...
QUnit.test('should be able to conditionally decode a single malformed cookie', function (assert) {
	...
	_Object$keys(cookies.get()).forEach(function (name) {
		cookies.remove(name, {
			path: ''
		});
	});
