...
(function () {
	...
	window.lifecycle = {
		afterEach: function afterEach() {
			_Object$keys(Cookies.get()).forEach(Cookies.remove);

			_Object$keys(Cookies.get()).forEach(function (cookie) {
			...
			return {
				then: function then(callback) {
					...
					if (!serverURL) {
						...
						var requestURL = [serverURL, 'encoding?', 'name=' + encodeURIComponent(name), '&value=' + encodeURIComponent(value)].join('');
						...
						addEvent(iframe, 'load', function () {
							...
							if (!content) {
								ok(false, ['"' + requestURL + '"', 'content should not be empty'].join(' '));
								done();
								return;
							}
							try {
